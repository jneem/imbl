...
use crate::{
    ...
    util::{Pool, PoolClone, PoolDefault, PoolRef},
};

use self::{Insert::*, InsertAction::*};
...
impl<A: BTreeValue> Node<A> {
        ...
        fn recurse<A: BTreeValue>(node: &Node<A>) -> (&A, &A) {
            ...
            if node.is_leaf() {
                ...
                for i in 0..node.keys.len() {
                    let left_max =
                        recurse(node.children[i].as_ref().unwrap()).1;
                    let right_min =
                        recurse(node.children[i + 1].as_ref().unwrap()).0;
                    assert!(
                        node.keys[i].cmp_values(left_max) == Ordering::Greater
                    );
                    assert!(
                        node.keys[i].cmp_values(right_min) == Ordering::Less
                    );
                }
                (
        ...
        match A::search_key(keys, key) {
            ...
            Err(index) => {
                self.children[index]
                    ...
                    .or_else(|| {
                        index.checked_sub(1).and_then(move |i| keys.get_mut(i))
                    })
            },
        }
    }
    ...
    fn split(
        ...
        match index.cmp(&MEDIAN) {
            Ordering::Less => {
                ...
                left_children =
                    Chunk::from_front(&mut self.children, index + 1);
                left_children.push_back(right_child);
                left_children
                    .drain_from_front(&mut self.children, MEDIAN - index - 1);
            ...
            Ordering::Greater => {
                ...
                left_children =
                    Chunk::from_front(&mut self.children, MEDIAN + 1);

                median = self.keys.pop_front();

                right_keys =
                    Chunk::from_front(&mut self.keys, index - MEDIAN - 1);
                right_keys.push_back(value);
                right_keys.append(&mut self.keys);

                right_children =
                    Chunk::from_front(&mut self.children, index - MEDIAN);
        ...
        A: Clone,
    {
        ...
        let (median, left, right) = match A::search_value(&self.keys, &value) {
            // Key exists in node
            Ok(index) => {
                return Insert::Replaced(mem::replace(
                    &mut self.keys[index],
                    value,
                ));
            },
        ...
        A::Key: Borrow<BK>,
    {
                ...
                match self.children[index] {
                    ...
                    None => {
                        match target {
                            ...
                            Err(Boundary::Highest) => {
                                RemoveAction::DeleteAt(self.keys.len() - 1)
                            },
                        }
                    },
                    ...
                    Some(ref child) if child.too_small() => {
                        ...
                        match (left, self.children.get(index + 1)) {
                            ...
                            (Some(&Some(ref old_left)), _)
                                if !old_left.too_small() =>
                            {
                                RemoveAction::StealFromLeft(index)
                            },
                            // If it has a right sibling with capacity, same as above.
                            (_, Some(&Some(ref old_right)))
                                if !old_right.too_small() =>
                            {
                            ...
                            (Some(&Some(_)), _) => {
                                RemoveAction::MergeFirst(index - 1)
                            },
        ...
        match action {
            ...
            RemoveAction::Merge(index) => {
                ...
                let right =
                    mem::replace(&mut self.children[index], None).unwrap();
                ...
                let (removed, new_child) =
                    match merged_child.remove_target(pool, target) {
                        Remove::NoChange => unreachable!(),
                        Remove::Removed(removed) => (removed, merged_child),
                        Remove::Update(removed, updated_child) => {
                            (removed, updated_child)
                        },
                    };
                if self.keys.is_empty() {
            ...
            RemoveAction::StealFromLeft(index) => {
                ...
                let out_value;
                {
                    let mut children = self.children.as_mut_slice()
                        [index - 1..=index]
                        .iter_mut()
                        .map(|n| n.as_mut().unwrap());
                    let left =
                        PoolRef::make_mut(pool, children.next().unwrap());
                    let child =
                        PoolRef::make_mut(pool, children.next().unwrap());
            ...
            RemoveAction::MergeFirst(index) => {
                if let Ok(key) = target {
                    ...
                    match self.keys[index].cmp_keys(key) {
                        Ordering::Less
                            if !self.child_contains(index + 1, key) =>
                        {
                            return Remove::NoChange;
                        },
                        Ordering::Greater
                            if !self.child_contains(index, key) =>
                        {
                            return Remove::NoChange;
                        },
                        _ => (),
                ...
                match merged.remove_target(pool, target) {
                    Remove::NoChange => {
                        panic!(
                            "nodes::btree::Node::remove: caught an absent key \
                             too late while merging"
                        );
                    },
                    Remove::Removed(value) => {
                        if self.keys.is_empty() {
        ...
        match Iter::get(&self.fwd_path) {
            ...
            Some(value) => {
                match Iter::get(&self.back_path) {
                    Some(last_value)
                        if value.cmp_values(last_value)
                            == Ordering::Greater =>
                    {
                        None
                    },
    ...
    fn next_back(&mut self) -> Option<Self::Item> {
        match Iter::get(&self.back_path) {
            ...
            Some(value) => {
                match Iter::get(&self.fwd_path) {
                    Some(last_value)
                        if value.cmp_values(last_value) == Ordering::Less =>
                    {
                        None
                    },
                    None => None,
...
impl<A: Clone> ConsumingIter<A> {
    ...
    fn push_node(
        ...
        if let Some(node) = maybe_node {
            stack.push(ConsumingIterItem::Consider(PoolRef::unwrap_or_clone(
                node,
            )))
        }
    }
    ...
    fn push_node_back(&mut self, maybe_node: Option<PoolRef<Node<A>>>) {
        if let Some(node) = maybe_node {
            self.back_stack.push(ConsumingIterItem::Consider(
                PoolRef::unwrap_or_clone(node),
            ))
        }
    }
                            ...
                            if !std::ptr::eq(old, new) {
                                match old.keys[0].cmp_values(&new.keys[0]) {
                                    Ordering::Less => {
                                        Self::push(&mut self.old_stack, old);
                                        self.new_stack
                                            .push(IterItem::Consider(new));
                                    },
                                    Ordering::Greater => {
                                        self.old_stack
                                            .push(IterItem::Consider(old));
                                        Self::push(&mut self.new_stack, new);
                                    },
                                    Ordering::Equal => {
                                        Self::push(&mut self.old_stack, old);
