...
where
    BitsImpl<WIDTH>: Bits,
{
    /// Nodes with  `WIDTH` < `HASH_WIDTH` small nodes.
    /// Small only contains `Value` entries.
    ///
    ...
    /// When in PureHamt mode, these are unused.
    ...
    BitsImpl<WIDTH>: Bits,
{
    ...
    fn ctrl_hash_and_group(hash: HashBits) -> (u8, usize) {
        ...
        let group = (hash >> (HashBits::BITS.saturating_sub(1))) as usize % GROUPS;
        (ctrl_hash, group)
    }
...
impl<A: HashValue, P: SharedPointerKind> SmallNode<A, P> {
        ...
        A::Key: Borrow<BK>,
    {
        ...
        let mut mask = group_find(&self.control[0], search);
    ...
    {
        let search = Self::ctrl_hash(hash);
        let mut bitmap = group_find(&self.control[0], search);
        ...
        // TODO: could also be performed slightly faster via the sparse chunk bitmap,
        // but such API isn't exposed. This is sufficient for now
        if let Some(index) = group_find_empty(&self.control[0]) {
            // If we found a free slot, we can insert the value
            self.control[0].as_mut_array()[index] = search;
        ...
        A::Key: Borrow<BK>,
    {
        ...
        while let Some(index) = bitmap.first_index() {
            if let Some(Entry::Value(value, value_hash)) = self.data.get(index) {
                if hash_may_eq::<A>(hash, *value_hash) && key == value.extract_key().borrow() {
                    self.control[0].as_mut_array()[index] = 0;
...
impl<A: HashValue, P: SharedPointerKind> Node<A, P> {
        ...
        A::Key: Borrow<BK>,
    {
        ...
        if mode == NodeMode::SimdGroups {
            ...
            let mut bitmap = group_find(&control[group], search);
            while let Some(offset) = bitmap.first_index() {
                let index = group * GROUP_WIDTH + offset;
                #[allow(unsafe_code)]
                let this = unsafe { &mut *this };
    ...
    fn merge_values(value1: A, hash1: HashBits, value2: A, hash2: HashBits) -> Entry<A, P> {
        let small_node = SmallNode::with(|node| {
            ...
            node.control[0].as_mut_array()[0] = Self::ctrl_hash(hash1);
            node.control[0].as_mut_array()[1] = Self::ctrl_hash(hash2);
        });
        ...
        A: Clone,
    {
        if self.mode == NodeMode::SimdGroups {
            ...
            if let Some(offset) = group_find_empty(&self.control[group]) {
                ...
                let index = group * GROUP_WIDTH + offset;
                self.data.insert(index, Entry::Value(value, hash));
                self.control[group].as_mut_array()[offset] = search;
                return None;
            }

            // Group is full, upgrade to PureHamt mode
            self.upgrade_to_hamt(shift);
            ...
            Entry::SmallNode(small_ref) => {
                ...
                match small.insert(hash, shift + HASH_SHIFT, value) {
                    ...
                    Err(value) => {
                        ...
                        *entry = Entry::Node(small.drain_to_node(hash, shift + HASH_SHIFT, value));
                        return None;
                    }
                }
            }
    ...
    #[cold]
    fn upgrade_to_hamt(&mut self, shift: usize)
    where
        A: Clone,
    {
        ...
        A::Key: Borrow<BK>,
    {
            ...
            while let Some(offset) = bitmap.first_index() {
                ...
                if let Some(Entry::Value(value, value_hash)) = self.data.get(index) {
                    if hash_may_eq::<A>(hash, *value_hash) && key == value.extract_key().borrow() {
                        ...
                        self.control[group].as_mut_array()[offset] = 0;
                        return removed;
                    }
                    bitmap.set(offset, false);
                } else {
        ...
        let new_node = match self.data.get_mut(index)? {
            ...
            Entry::SmallNode(small_ref) => {
                let small = SharedPointer::make_mut(small_ref);
                removed = small.remove(hash, shift + HASH_SHIFT, key);
                (small.len() == 1).then(|| small.pop())
            }
